...
export default {
    ...
    data() {
        return {
            categories: [
                ...
                {
                    title: "Servers & Version Control",
                    skills: [
                        {
                            title: "AWS (EC2, EB, RDS, SQS)",
                            logo: "/images/aws.png",
                            percent: 70,
                        },
                        {
    ...
    methods: {
        divideIntoColumns(skills, second = false, test = "") {
            const half = Math.ceil(skills.length / 2);

            if (second) {
                if (skills.length === 2) {
                    return skills.slice(half);
                }
